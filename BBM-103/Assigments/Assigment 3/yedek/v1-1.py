...
def findSameValue(event, cordinate):
   ...
   queue = [{"row": row, "colon": colon}]  # Komşuları kontrol etmek üzere bir kuyruk oluştur

   neighbors = set()
   while queue:
      current = queue.pop(0)  # Kuyruğun başındaki elemanı al
   ...
   return neighbors  # Birbirine komşu olan tüm koordinatların setini döndür
...
def main():
   ...
   while(1<2):
      if(sustainabilityGame(game)==0):
         print(f"your scorre is: {score}")
         print("Game over")    
         break
      gamerInput = input("Please enter a colon and a column number: ")

      cordinate = inputVeriable(gamerInput)
      if((cordinate["row"]>=row or cordinate["row"]<0)or(cordinate["colon"]>=row or cordinate["colon"]<0) ):
      ...
      scoreStep=(len(neighbors))*(gameTemplate[row-1][colon-1])
      game=changeAndTakeScore(game,neighbors,row,colon)
      score+=scoreStep
      printConsole(game,score)
